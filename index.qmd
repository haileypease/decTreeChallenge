...
See @sec-introduction for the discussion questions.
...
## Discussion Questions for Challenge {#sec-discussion-questions}
...
A zip code should be modelled as a catgorical variable. Even though zip codes are represented as numbers, you cannot have negative zip codes, add zip codes, find average zip codes, etc. If my home zip code is 08094 and my zipcode at school is 19711, it means nothing to add them or compare the two. 19711 zimply means the Newark area in Delaware, and 08094 means the Williamstown area in New Jersey. When you are making decisions with categorical data, it is not about the numbers, it is about the categories. For zip codes, it could mean, "Do I want to live in the Williamstown area or the Newark area?" Not the area that is 19711 because it is a higher number. When it comes to the zip codes, it is more about the frequency of the zip codes in the area rather than the numbers themselves. 
...
The reason for the difference is when using the Python code (numerical), the decision tree took into account the zip codes that were not clearly categorized. For instance, the zip code 19711.11 could be used within the analysis, even if there is no clear area that aligns with this zip code. When using the R code (categorical), all the numbers are properly categorized, so the decision tree did not take into account the zip codes that were not clearly categorized. That is why the R code actually has less values than the Python code. The code that shows the R Studio code is doing a better job is:
...
This shows the importance feature which creates the best split for the decision tree.
